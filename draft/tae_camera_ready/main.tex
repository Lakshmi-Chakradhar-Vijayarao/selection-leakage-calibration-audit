...
\title{Selection-Induced Optimism Corrupts Calibration, Not Just Discrimination:
...
all nine survive joint family-wise correction; a regularization sweep leaves
the Brier gap shrunk, the Mechanism-1 ECE gap gone, and the risk violation
intact. Calibration is checked on three of five mechanisms and established on
two, on one model family across two question pools; the four additional
architectures test the AUROC selection component only, not calibration.
...
same selection step also corrupts \emph{calibration}. We show it does: a probe
...
\begin{itemize}
\itemsep0pt
\item The first evidence that selection-induced optimism corrupts
   calibration and degrades an actual selective-prediction operating
...
\paragraph{What each claim is, and how strongly.} Every claim sits in one of
three tiers and is read at that tier, nowhere at a stronger one:
\textbf{(A) documented} -- present in the harnesses as written, descriptive;
\textbf{(B) statistically established} -- BCa $95\%$ excludes zero,
Holm-corrected where a family applies; \textbf{(C) illustrative, unresolved, or
negative} -- reported but not relied on, including our own primary synthetic
estimate, which fails its own control. We give no prevalence estimate:
repositories were found by targeted search, not sampled.
Table~\ref{tab:claim-tiers} (Appendix~\ref{app:claim-tiers}) maps every row.
...
\section{Operational Safety Failures: Selection Bias Breaks Selective Prediction}
...
Brier) follows from its degenerate construction, not from selection. We check
calibration on three of the five mechanisms and establish it on two -- on
Mechanism 3's real-feature harness neither gap is BCa-established -- and do not
claim it generalizes to Mechanisms 4--5 without checking each directly.
Mechanism 1's probe interpolates at $C=1.0$; at $C=0.01$ its Brier gap survives
shrunk ($4\times$), \textbf{its ECE gap does not}, and Mechanism 2's cells,
which never interpolate, are untouched (Appendix~\ref{app:reg}). The direction
matches every discrimination-metric finding here: the selection step that
inflates AUROC also makes the resulting confidence estimates look more
...
verdict above as the conservative one (Appendix~\ref{app:reviewer-response}).
What degrades here is the abstention rate a deployment was sized against: this
transfers an empirically chosen threshold and neither constructs nor
contradicts a distribution-free risk-controlling procedure.
...
Calibration is checked on three of five mechanisms and established on two, and
roughly half the gap is ordinary reuse rather than selection. Mechanism 1's
probe sits near interpolation: under regularization the risk violation is
stable but its ECE gap does \emph{not} survive. The cross-architecture check
uses forced-choice caches and tests discrimination only. We give no prevalence
estimate, the severity surface is a fit rather than a bound, and one case
study's headline number is not independently re-verifiable and is excluded
from every comparison. Appendix~\ref{app:limitations} has these in full.
...
Scope limits & Calibration shown on two mechanisms (the four extra
  architectures test AUROC only); M2 at $30\%$ coverage established only under
  the coverage-aware count; severity relationship diagnostic, not predictive
