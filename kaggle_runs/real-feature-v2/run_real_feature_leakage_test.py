...
This extracts real features from the ACTUAL audited pipeline's exact
feature-extraction code (`extract_features`, copied verbatim from
MultiHaluDet's public repo, src/data/feature_extractor.py -- unmodified,
since it is already model-agnostic: it just calls
...
if not torch.cuda.is_available():
    raise RuntimeError("No CUDA device visible -- this kernel needs a T4 accelerator (AWQ requires a compatible GPU; there is no CPU fallback for a 7B AWQ model).")
try:
    ...
    raise RuntimeError(f"GPU sanity check failed on {torch.cuda.get_device_name(0)} -- likely an incompatible accelerator (e.g. P100) was assigned instead of T4. Re-push and retry. Original error: {e}")
...
# ── MultiHaluDet's own feature-extraction code, copied verbatim from
#    src/data/feature_extractor.py (public repo) -- unmodified. ──────────────
...
CAPACITY = 128        # this paper's flagship capacity (02d)
